...
// Vista de solo lectura para un funcionario sin gente a cargo: ve sus
// propias inconsistencias y qué le falta mandarle a su jefatura por interno
// para que ella las regularice en la app — nunca puede editar nada acá.
export function VistaFuncionario({ session, onSalir }: { session: Session; onSalir: () => void }) {
  const [casos, setCasos] = useState<Caso[] | null>(null);
  const [error, setError] = useState('');

  useEffect(() => {
    api
      .get<{ casos: Caso[] }>('/api/casos')
  ...
  return (
    <div style={{ minHeight: '100vh', fontFamily: 'var(--font-body)', color: 'var(--color-text)' }}>
      ...
      <main style={{ maxWidth: 980, margin: '0 auto', padding: '32px 32px 90px' }}>
        <h1 style={{ fontSize: 30, margin: '0 0 8px' }}>Sus inconsistencias de marcaje</h1>
        <p className="text-muted" style={{ fontSize: 13.5, maxWidth: '68ch', margin: '0 0 24px' }}>
          ...
          información indicada en <strong>Acción solicitada</strong> a su jefatura por conducto interno — es ella quien
          la regulariza en el sistema.
        </p>
